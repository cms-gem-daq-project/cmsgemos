  ...
  try {
    ...
    if (destDsc->getClassName().find("tcds") != std::string::npos) {
      ...
      tcdscmd << cmd << " xdaq:actionRequestorId=\""
              << srcDsc->getClassName() << "\"";
      DEBUG("::sendTCDSCommand '" << tcdscmd.str() << "'" << " to '" << destDsc->getClassName() << "'");
  ...
  throw (gem::utils::exception::Exception)
{
  ...
  try {
    ...
    // xoap::dumpTree(msg->getSOAPPart().getEnvelope().getDOMNode(),tool);
    msg->writeTo(tool);
    DEBUG("::sendTCDSCommand '" << cmd << "': SOAP " << tool);
    xoap::MessageReference reply = appCxt->postSOAP(msg, *srcDsc, *destDsc);
    // tool.clear();
    // xoap::dumpTree(reply->getSOAPPart().getEnvelope().getDOMNode(),tool);
  } catch (xcept::Exception& e) {
...
bool gem::utils::soap::GEMSOAPToolBox::sendCommandWithParameterBag(std::string const& cmd,
  ...
  try {
    ...
    if (destDsc->getClassName().find("tcds") != std::string::npos) {
      ...
      DEBUG("::sendCommandWithParameterBag: '" << cmd << "' adding attribute " << cmdtype.getQualifiedName());
    }
    ...
    DEBUG("::sendCommandWithParameterBag Inspecting command elements with qualified name "
    ...
    for (std::vector<xoap::SOAPElement>::iterator elem = elems.begin(); elem != elems.end(); ++elem) {
      DEBUG("::sendCommandWithParameterBag found element elem != elems.end() ? " << (elem != elems.end()));
      DEBUG("::sendCommandWithParameterBag  qualifiedName: "
            << elem->getElementName().getQualifiedName());
      DEBUG("::sendCommandWithParameterBag  prefix: "
            << elem->getElementName().getPrefix());
      DEBUG("::sendCommandWithParameterBag  localName: "
            << elem->getElementName().getLocalName());
      DEBUG("::sendCommandWithParameterBag  URI: "
            << elem->getElementName().getURI());
      // // DEBUG("::sendCommandWithParameterBag memory address: "
      // //       << std::hex << elem << std::dec);
      DEBUG("::sendCommandWithParameterBag  has value: "
            << elem->getValue());
      DEBUG("::sendCommandWithParameterBag and has "
            << elem->getChildElements().size() << " subchildren");
    }
    std::string tool;
    xoap::dumpTree(msg->getSOAPPart().getEnvelope().getDOMNode(),tool);
    DEBUG("::sendCommandWithParameterBag: SOAP message is: " << tool);
                                                                  ...
                                                                  xdaq::ApplicationDescriptor* destDsc)
{
  ...
  try {
    ...
    if (basic.size() == 1) {
      ...
      DEBUG("::createStateRequestMessage application " << destDsc->getClassName()
