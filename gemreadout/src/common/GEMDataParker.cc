...
uint32_t* gem::readout::GEMDataParker::getGLIBData(uint8_t const& gtx, uint32_t counter[5])
{
  ...
  CMSGEMOS_DEBUG(" ::getGLIBData Starting while loop readout " << whileStart
  ...
  while ( p_glibDevice->getFIFOVFATBlockOccupancy(gtx) ) {
    ...
    CMSGEMOS_DEBUG(" ::getGLIBData initiating call to getTrackingData(gtx,"
          << p_glibDevice->getFIFOVFATBlockOccupancy(gtx) << ") "
          << getTrackingStart);
    std::vector<uint32_t> data = p_glibDevice->getTrackingData(gtx,
                                                               p_glibDevice->getFIFOVFATBlockOccupancy(gtx));
    Float_t getTrackingFinish = (Float_t)timer.RealTime();
    CMSGEMOS_DEBUG(" ::getGLIBData The time for one call of getTrackingData(gtx) " << getTrackingFinish
          << std::endl << "FIFO VFAT block depth 0x" << std::hex
          << p_glibDevice->getFIFOVFATBlockOccupancy(gtx)
          << std::endl << "FIFO depth 0x" << std::hex
          << p_glibDevice->getFIFOOccupancy(gtx)
  ...
  CMSGEMOS_DEBUG(" ::getGLIBData The time for while loop execution " << whileFinish
        << std::endl
        << " FIFO VFAT block occupancy  0x" << std::hex << p_glibDevice->getFIFOVFATBlockOccupancy(gtx)
        << std::endl
        << " FIFO occupancy             0x" << std::hex << p_glibDevice->getFIFOOccupancy(gtx) << std::endl
...
{
  for(int j = 0; j < 5; j++) {
    CMSGEMOS_INFO("GEMDataParker::selectData counter " << j <<  " "<< counter[j] );
  }
  uint32_t *point = &counter[0];
  CMSGEMOS_DEBUG("GEMDataParker::selectData point  " << std::hex << point );
...
uint32_t* gem::readout::GEMDataParker::GEMEventMaker(uint32_t counter[5])
{
  ...
  CMSGEMOS_DEBUG("GEMDataParker::GEMEventMaker  " << std::hex << point );
  ...
  if (islot < 0 || islot > 23) {
    ...
    CMSGEMOS_DEBUG(" ::GEMEventMaker warning !!! islot is undefined " << islot << " erros.size " << int(erros.size()) );
  } else {
    // VFATs Pay Load
    if ( int(vfats.size()) <= MaxVFATS ) vfats.push_back(vfat);
    CMSGEMOS_DEBUG(" ::GEMEventMaker m_event " << m_event << " vfats.size " << vfats.size() << std::hex << " ES 0x" << ES << std::dec );
...
void gem::readout::GEMDataParker::GEMevSelector(const  uint32_t& ES)
{
  ...
  CMSGEMOS_DEBUG(" ::GEMEventMaker vfats.size " << int(vfats.size()) << " rvent_ " << rvent_ << " event " << m_event);
  ...
  for (auto iVFAT=vfats.begin(); iVFAT != vfats.end(); ++iVFAT) {//try auto
    ...
    if ( ES == localEvent ) {
      ...
      CMSGEMOS_DEBUG(" ::GEMevSelector slot number " << islot );
  ...
  // contents all local events (one buffer, all links):
  CMSGEMOS_DEBUG(" ::GEMEventMaker END ES 0x" << std::hex << ES << std::dec << " errES " <<  erros.size() <<
        " rvent_ " << rvent_ );
  ...
  if (m_event%kUPDATE == 0 &&  m_event != 0) {
    CMSGEMOS_DEBUG(" ::GEMEventMaker vfats.size " << std::setfill(' ') << std::setw(7) << int(vfats.size()) <<
...
bool gem::readout::GEMDataParker::VFATfillData(int const& islot, AMCGEBData&  geb)
{
  ...
  CMSGEMOS_DEBUG(" ::VFATfillData ChamID 0x" << ChamID << std::dec << " islot " << islot << " sumVFAT " << sumVFAT);
...
void gem::readout::GEMDataParker::ScanRoutines(uint8_t latency, uint8_t VT1, uint8_t VT2)
{
  ...
  m_VT1 = VT1;
  m_VT2 = VT2;
  CMSGEMOS_DEBUG("GEMDataParker::ScanRoutines Latency = " << (int)m_latency  << " VT1 = " << (int)m_VT1 << " VT2 = " << (int)m_VT2);
}
