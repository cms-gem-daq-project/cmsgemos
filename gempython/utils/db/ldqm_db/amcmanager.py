...
class AMCmanager:
  ...
  def checkGTX(self, device, link):
    fwv = getFirmwareVersionRaw(device,link)
    if fwv != 0x0:
      return True
    else:
      return False
  ...
  def getVFATs(self,device,gtx):
    return getAllChipIDs(device,gtx)
