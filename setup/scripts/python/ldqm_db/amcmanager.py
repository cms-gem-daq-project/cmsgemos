...
class AMCmanager:
  ...
  def getStatus(self, verbosity):
    ...
    self.status += '<tr><th>DAV TIMEOUT</th><td>0x%08X</td><td>0x%08X</td></tr>' % (readRegister(self.glib,"GLIB.DAQ.GTX0.CONTROL.DAV_TIMEOUT"),readRegister(self.glib,"GLIB.DAQ.GTX1.CONTROL.DAV_TIMEOUT"))
    self. status += '</table>'
    #DEBUG TABLE
    if verbosity>1:
