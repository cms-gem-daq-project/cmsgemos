...
throw (xgi::exception::Exception)
{
	...
	*out << "      <div class=\"xdaq-tab\" title=\"XML Test Page\"/>"  << std::endl;
	xmlTestPage(in, out);
	*out << "      </div>" << std::endl;
	*out << "    </div>" << std::endl
		<< cgicc::span()     << std::endl
	...
	void gem::supervisor::GEMSupervisorWeb::xmlTestPage(xgi::Input * in, xgi::Output * out)
throw (xgi::exception::Exception)
{
	DEBUG("GEMSupervisorWeb::rendering GEMSupervisorWeb xmlTestPage");
	// should put all this style information into a common CSS sheet
	*out << cgicc::section().set("style", "display:inline-block;float:left") << std::endl
		<< cgicc::fieldset().set("style", "style=\"display:block\";padding:5px;margin:5px;list-style-type:none;margin-bottom:5px;line-height:18px;padding:2px 5px;-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px;border:medium outset #CCC;")
		<< std::endl
		<< cgicc::legend("GEM Supervisor xml Test page")    << std::endl
		<< cgicc::br()       << std::endl
		<< cgicc::span()     << std::endl
		<< "<button class=\"hide\" id=\"xmlTest\" onclick=\"xmlTestCommand(\'Test\',\'/"
		<< p_gemApp->m_urn << "\')\">XML Test</button>"
		<< cgicc::br() << std::endl
		<< cgicc::fieldset() << std::endl
		<< cgicc::section()  << std::endl;
}
